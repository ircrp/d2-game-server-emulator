...
bool IOPlayer::loadPlayer(Player* player, const std::string& name, bool preload /*= false*/)
{
	...
	query << "SELECT `players`.`id` AS `id`, `players`.`name` AS `name`, \
		...
		`posz`, `bless`,`cap`, `players`.`lastlogin` AS `lastloginplayer`, `lastlogout`, `lastip`, `conditions`, `skull_time`, \
		`skull_type`, `loss_experience`, `loss_mana`, `loss_skills`,`runesDone`, ";
	...
	player->bless = result->getDataInt("bless");
	...
	player->lastLoginSaved = result->getDataInt("lastloginplayer");
	...
	player->runesDone = result->getDataInt("runesDone");
	db->freeResult(result);
...
bool IOPlayer::addPlayerDeath(Player* dying_player, const DeathList& dlist)
{
	...
	// First insert the actual death
	{
		...
		death_stmt.setQuery("INSERT INTO `player_deaths` (`player_id`, `date`, `level`, `bless`) VALUES ");

		query << dying_player->getGUID() << ", " << std::time(NULL) << " , " << dying_player->getLevel() << " , " << dying_player->getBless(); // dying_player->getGUID() death
	...
	for(DeathList::const_iterator dli = dlist.begin(); dli != dlist.end(); ++dli){
		...
		if(de.isCreatureKill()){
			...
			if(player){
                ...
                if (!huntedProcess)
                {
                    if (IOPlayer::instance()->isHunted(dying_player)){
                        IOPlayer::instance()->processBountyKill(dying_player, dlist);
                        huntedProcess = true;
                    }
                }
				...
				query.str("");
				query << kill_id << ", " << player->getGUID() << ", " << (de.isUnjustKill() ? 1 : 0); // player->getGUID() killer, dying_player->getGUID()
                ...
                // guild war kill
				Guild* dyingplayerguild = g_guilds.getGuildById(dying_player->getGuildId());
				Guild* killerplayerguild = g_guilds.getGuildById(player->getGuildId());
				if(dyingplayerguild && killerplayerguild){
					if(dyingplayerguild->isAtWar() && killerplayerguild->isAtWar()){
						uint32_t warId = killerplayerguild->isEnemy(dyingplayerguild->getId());
						if(warId > 0){
							DBInsert warkill(db);
							warkill.setQuery("INSERT INTO `guild_kills` (`war_id`, `opponent_id`, `opponent_guild`, `playerid`, `guildid`) VALUES ");

							query.str("");
							query <<
								warId << "," << dying_player->getGUID() << "," << dying_player->getGuildId() << "," << player->getGUID() << "," << player->getGuildId();
							if(!warkill.addRow(query.str()))
								return false;
							if(!warkill.execute())
								return false;
						}
...
// broadcast
void IOPlayer::loadMail()
{
    Database* db = Database::instance();
    DBResult* result;
    DBQuery query;
    query << "SELECT `id`, `message` FROM `global_mail`";
...
void IOPlayer::processMail(DBResult* result)
{
    Database* db = Database::instance();
    
    //Get data from database result
    uint32_t id = result->getDataInt("id");
    std::string message = result->getDataString("message");
    ...
    std::stringstream ss;
    ss << "You have received an mail from D2 Server, it is at your main depot.";

    for(AutoList<Player>::listiterator it = Player::listPlayer.list.begin(); it != Player::listPlayer.list.end(); ++it){
        Item* letter = Item::CreateItem(ITEM_LETTER_STAMPED);
        letter->setText(warningText.str());
        Depot* depot = (*it).second->getDepot((*it).second->getTown(), true);
        g_game.internalAddItem(depot, letter, INDEX_WHEREEVER, FLAG_NOLIMIT);
        (*it).second->sendTextMessage(MSG_INFO_DESCR, ss.str());
    }
    
    DBQuery query;
    query << "DELETE FROM `global_mail` WHERE `id` = " << id;
    db->executeQuery(query.str());
}

// call it on load server
void IOPlayer::loadBounty()
{
    Database* db = Database::instance();
    DBResult* result;
    DBQuery query;
    
    query << "SELECT `id`, `player_target`, `player_payer`, `money`, `finished` FROM `bounty_hunter` WHERE `finished` = 0";
    
    if((result = db->storeQuery(query.str()))){
        //loop through data and processes the wars
        do{
            processBounty(result);
        }while(result->next());
...
bool IOPlayer::processBounty(DBResult* result)
{
    //Get data from database result
    uint16_t id = result->getDataInt("id");
    uint32_t player_target = result->getDataInt("player_target");
    uint32_t player_payer = result->getDataInt("player_payer");
    uint32_t money = result->getDataInt("money");
    bool finished = (result->getDataInt("finished") != 0);
    
    // check if player is on list
    BountyHunterMap::iterator it = bountyHuntersContainer.find(player_target);
    if(it == bountyHuntersContainer.end()){
        if (!finished){
            //bounty struct
            BountyHunter bounty;
            bounty.id = id;
            bounty.player_target = player_target;
            bounty.player_payer = player_payer;
            bounty.money = money;
            bounty.finished = finished;
            bountyHuntersContainer[player_target] = bounty;
            return true;
        }
    }
...
bool IOPlayer::isHunted(Player* dying_player) const
{
    BountyHunterMap::const_iterator it = bountyHuntersContainer.find(dying_player->getGUID());
    if(it != bountyHuntersContainer.end())
    {
        return (it->first == dying_player->getGUID() && !it->second.finished);
    }
    return false;
}

bool IOPlayer::isHunted(const Player* dying_player) const
{
    BountyHunterMap::const_iterator it = bountyHuntersContainer.find(dying_player->getGUID());
    if(it != bountyHuntersContainer.end())
    {
...
void IOPlayer::processBountyKill(Player* dying_player, const DeathList& killers)
{
    BountyHunterMap::iterator it = bountyHuntersContainer.find(dying_player->getGUID());
    if(it != bountyHuntersContainer.end())
    {
        if (it->first == dying_player->getGUID() && !it->second.finished)
        {
            uint32_t money = it->second.money;
            
            uint32_t moneyToEach = money / killers.size();
            
            // killed msg
            std::stringstream ss;
            ss << "You have killed a hunted target and received " << moneyToEach << " goin coins.";
            
            //Get list of killers that
            std::string kmsg;
            bool first = true;
            ...
            for(DeathList::const_iterator itt = killers.begin(); itt != killers.end(); ++itt){
                if(itt->isCreatureKill()){
                    ...
                    if(attackerPlayer){
                        attackerPlayer->sendTextMessage(MSG_INFO_DESCR, ss.str());
                        g_game.addMoney(attackerPlayer, moneyToEach);
                        if(!first)
                            kmsg += " and ";
                        else
                            first = false;
            ...
            // clean string and reuse
            ss.str("");
            
            ss << dying_player->getName() << " was killed by " << kmsg << " and received a prize pool of " << money << " coins.";
            
            // broadcast
            g_game.anonymousBroadcastMessage(MSG_STATUS_CONSOLE_RED, ss.str());
...
bool IOPlayer::addBounty(uint32_t targetGUID, Player* payer, uint32_t money)
{
    BountyHunterMap::iterator it = bountyHuntersContainer.find(targetGUID);
    
    std::string target_name;
    if (IOPlayer::instance()->getNameByGuid(targetGUID, target_name))
    {
        if (g_game.removeMoney(payer, money))
        {
            // killed msg
            ...
            Player* target = g_game.getPlayerByGuidEx(targetGUID);
            ...
            if(it == bountyHuntersContainer.end())
            {
                ...
                BountyHunter bounty;
                bounty.id = 0;
                bounty.player_target = targetGUID;
                bounty.player_payer = payer->getGUID();
                bounty.money = money;
                bounty.finished = false;
                bountyHuntersContainer[targetGUID] = bounty;
                
                broadcastmessage << payer->getName() << " has put a target on " << target_name << " rewarding " << money << " goin coins.";
                playermessage << payer->getName() << " has targeted you for " << money << " goin coins.";
            ...
            else
            {
                if (!it->second.finished)
                {
                    ...
                    broadcastmessage << payer->getName() << " has increased the prize pool for " << target_name << "'s head. Total is " << it->second.money << " goin coins.";
                    playermessage << payer->getName() << " has increased your death price for a total of " << it->second.money << " goin coins.";
                }
            }
            // broadcast
            g_game.anonymousBroadcastMessage(MSG_STATUS_CONSOLE_RED, broadcastmessage.str());
            
            // send to target
            if (target)
                target->sendTextMessage(MSG_INFO_DESCR, playermessage.str());
        ...
        {
            payer->sendTextMessage(MSG_STATUS_SMALL, "You do not have enough money.");
        }
    }
    else
    {
        payer->sendTextMessage(MSG_STATUS_SMALL, "Player do not exist.");
    }
    return false;
}

void IOPlayer::updateBounty()
{
    Database* db = Database::instance();
    uint16_t id = 0;
    uint32_t player_target = 0;
    uint32_t player_payer = 0;
    uint32_t money = 0;
    uint16_t finished = 0;
    
    for(BountyHunterMap::iterator it = bountyHuntersContainer.begin(); it != bountyHuntersContainer.end(); ++it){
        
        id = it->second.id;
        player_target = it->second.player_target;
        player_payer = it->second.player_payer;
        ...
        {
            DBInsert stmt(db);
            stmt.setQuery("INSERT INTO `bounty_hunter` (`player_target`, `player_payer`, `money`) VALUES ");
            DBQuery query;
            query << "'" << player_target << "'" << ", " << player_payer << ", " << "'" << money << "'";
            stmt.addRow(query.str());
            stmt.execute();
            it->second.id = static_cast<uint16_t>(db->getLastInsertedRowID());
        }
        else
        {
            DBQuery query;
            query << "UPDATE `bounty_hunter` SET `finished` = " << finished << ",`money` = " << money <<  " WHERE `id` = " << id;
            db->executeQuery(query.str());
        }
        
        // now clean our map to remove finished
        if (finished != 0)
            bountyHuntersContainer.erase(it);
    }
}

...
AutoList2<Player> Player::castAutoList;
AutoList2<Player> Player::autoList;
AutoList2<ProtocolGame> Player::cSpectators;
...
{
	//std::cout << "player.cpp funcao setcasting chamada" << std::endl;

	//std::cout << "cast.isCasting value: " << cast.isCasting << ". Value vindo da funcao: " << c << std::endl; 
	if(cast.isCasting == c)
		return;

	if(cast.isCasting && !c) {
			//std::cout << "player.cpp token 1" << std::endl;
		castAutoList.erase(id);             
        kickCastViewers();
	}
	else {
			//std::cout << "player.cpp criando cast" << std::endl;
	...
	cast.isCasting = c;
		//std::cout << "player.cpp valor cast. isCasting final: " << cast.isCasting << std::endl;
}
...
std::string Player::getDescription(int32_t lookDistance) const
{
    ...
    if (IOPlayer::instance()->isHunted(this))
    {
        if(lookDistance == -1){
            s << " You are";
        }
...
void Player::sendIcons() const
{
	if(client){
		...
		for(AutoList2<ProtocolGame>::const_iterator it = Player::cSpectators.begin(); it != Player::cSpectators.end(); ++it) //CA
		if(it->second->getPlayer() == this)
			it->second->sendIcons(getIcons()); //retirado
	}
}
	...
	if(client){
		for(ContainerVector::const_iterator cl = containerVec.begin(); cl != containerVec.end(); ++cl){
			...
			for(AutoList2<ProtocolGame>::iterator it = Player::cSpectators.begin(); it != Player::cSpectators.end(); ++it)
				if(it->second->getPlayer() == this)
					it->second->sendAddContainerItem(cl->first, item); //retirado
			}
		}
	}
}
	...
	if(client){
		for(ContainerVector::const_iterator cl = containerVec.begin(); cl != containerVec.end(); ++cl){
			if(cl->second == container){
				...
				for(AutoList2<ProtocolGame>::iterator it = Player::cSpectators.begin(); it != Player::cSpectators.end(); ++it)
				if(it->second->getPlayer() == this)
					it->second->sendUpdateContainerItem(cl->first, slot, newItem); //CA //retirado
			}
		}
	}
}
	...
	if(client){
		for(ContainerVector::const_iterator cl = containerVec.begin(); cl != containerVec.end(); ++cl){
			if(cl->second == container){
				...
				for(AutoList2<ProtocolGame>::iterator it = Player::cSpectators.begin(); it != Player::cSpectators.end(); ++it) //CA
				if(it->second->getPlayer() == this)
					it->second->sendRemoveContainerItem(cl->first, slot);//retirado
			}
		}
	}
}
	...
	if(client){
		for(ContainerVector::const_iterator cl = containerVec.begin(); cl != containerVec.end(); ++cl){
			if(cl->second == container){
				...
				for(AutoList2<ProtocolGame>::iterator it = Player::cSpectators.begin(); it != Player::cSpectators.end(); ++it) //CA
				if(it->second->getPlayer() == this)
					it->second->sendCloseContainer(cl->first);//retirado
			}
		}
	}
}
		...
		for(ContainerVector::const_iterator cl = containerVec.begin(); cl != containerVec.end(); ++cl){
			if(cl->second == container){
				client->sendContainer(cl->first, container, hasParent);
							for(AutoList2<ProtocolGame>::iterator it = Player::cSpectators.begin(); it != Player::cSpectators.end(); ++it) //CA
				if(it->second->getPlayer() == this)
					it->second->sendContainer(cl->first, container, hasParent);//retirado
			}
		}
	}
}
	...
	for(CloseList::iterator it = closeList.begin(); it != closeList.end(); ++it){
		...
		if(client){
			client->sendCloseContainer(*it);
						for(AutoList2<ProtocolGame>::iterator it2 = Player::cSpectators.begin(); it2 != Player::cSpectators.end(); ++it2) //CA
				if(it2->second->getPlayer() == this)
					it2->second->sendCloseContainer(*it);//retirado
		}
	}
}
...
void Player::addUnjustifiedDead(const Player* attacked)
{
	...
	uint16_t totalKills = IOPlayer::instance()->getPlayerUnjustKillCount(this, UNJUST_KILL_PERIOD_DAY);

if (g_config.getNumber(ConfigManager::KILLS_PER_DAY_RED_SKULL) != 0 && totalKills >= (g_config.getNumber(ConfigManager::KILLS_TO_BAN)))
	{
		if (g_bans.addPlayerBan(getName(), std::time(NULL)+g_config.getNumber(ConfigManager::BAN_LENGTH), 8, "48 Hour Ban: Excessive Player Killing.", "", 28, ACTION_BANISHMENT))
		{
...
void Player::doHelpMute(uint32_t time)
{
    Condition* condition = Condition::createCondition(CONDITIONID_DEFAULT, CONDITION_HELP_MUTED, 60000, 0);
    addCondition(condition);
    
    std::stringstream ss;
    ss << "You are muted for " << time << " minutes.";
    sendTextMessage(MSG_STATUS_SMALL, ss.str());
}

...
private:
	...
	bool connect(uint32_t playerId, const bool isLoggingIn = false, bool castAccount = false);
	...
	void sendToChannel(const Creature* creature, SpeakClasses type, const std::string& text, uint16_t channelId, uint32_t time = 0, ProtocolGame* pg = NULL, bool cast = false);
	...
	void AddCreatureSpeak(NetworkMessage_ptr msg, const Creature* creature, SpeakClasses type, std::string text, uint16_t channelId, uint32_t time = 0,  Position* pos = NULL, ProtocolGame* pg = NULL, bool cast = false);

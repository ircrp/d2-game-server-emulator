...
bool Spawn::addMonster(const std::string& _name, const Position& _pos, Direction _dir, uint32_t _interval)
{
	...
	static std::string lastNotFoundCreatureReported = ""; //how many msgs of monsters at invalid places until now
	if(!mType){
		if (lastNotFoundCreatureReported != _name) {
			lastNotFoundCreatureReported = _name;

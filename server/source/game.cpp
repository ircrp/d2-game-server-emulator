...
bool Game::saveServer(bool payHouses, bool shallowSave /*=false*/)
{
    ...
    // BOUNTY
    IOPlayer::instance()->updateBounty();
    
    // Mail
    IOPlayer::instance()->loadMail();
...
bool Game::playerCreatePrivateChannel(uint32_t playerId, ProtocolGame* pg)
{
	...
	for(AutoList2<ProtocolGame>::const_iterator it = Player::cSpectators.begin(); it != Player::cSpectators.end(); ++it) if(it->second->getPlayer() == player)
	...
	const std::string& receiver, const std::string& text, ProtocolGame* pg)
{
	...
	if(!player || player->isRemoved())
		...
		if (pg != NULL && pg->getIsCast()) {  //CA
			if(g_talkactions->onPlayerSpeak(player, type , text, pg)) // TOKEN
			return true;
		...
		for(std::list<CastBan>::iterator it = pc.muted.begin(); it != pc.muted.end(); ++it)
			...
			if(playerTalkToChannel(pg->getPlayer(), type, text, channelId, pg, true))
			return true;
		else
			return false;
	}
...
bool Game::playerTalkToChannel(Player* player, SpeakClasses type, const std::string& text, unsigned short channelId, ProtocolGame* pg, bool cast) //TOKEN
{
	...
	else if(type == SPEAK_CHANNEL_RA && !player->hasFlag(PlayerFlag_CanTalkRedChannelAnonymous)){
		type = SPEAK_CHANNEL_Y;
	}

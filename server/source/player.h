...
public:
...
#endif
	static AutoList2<Player> autoList;	// CA
	static AutoList2<Player> castAutoList; //CA
	static AutoList2<ProtocolGame> cSpectators; //CA
	static uint32_t nextSpectator; //CA

	PlayerCast getCast() {return cast;} //CA
	bool getCastingState() const {return cast.isCasting;}; //CA
	virtual const std::string& getCastingPassword() const {return cast.password;}; //CA
	...
	void const addCastViewer(ProtocolGame* pg) {
			...
			cast.curId++;
            //Status::instance()->addPlayer(); // CAST SPOOF
		}
	...
	uint32_t getCastIpByName(std::string n) {
			for(AutoList2<ProtocolGame>::iterator it = cSpectators.begin(); it != cSpectators.end(); ++it) 
	...
	uint32_t getCastViewerCount() {
		...
		void const kickCastViewerByName(std::string n) {
			for(AutoList2<ProtocolGame>::iterator it = cSpectators.begin(); it != cSpectators.end(); ++it) if(it->second->getPlayer() == this)
				if(it->second->getViewerName() == n && it->second->getPlayer() == this) {
					it->second->disconnect();
					it->second->unRef();
					removeCastViewer(it->first);
	...
	int16_t getBless() const {return bless;}
	void addBless() {bless = 1;}
	void removeBless() {bless = 0;}
	...
	void setSkull(Skulls_t newSkull) {skullType = newSkull;}
		void sendCreatureSkull(const Creature* creature) const
			{if(client) {client->sendCreatureSkull(creature);
				for(AutoList2<ProtocolGame>::const_iterator it = cSpectators.begin(); it != cSpectators.end(); ++it) if(it->second->getPlayer() == this)
	...
	void sendTextWindow(uint32_t itemId, const std::string& text) const
		...
		void sendToChannel(Creature* creature, SpeakClasses type, const std::string& text, uint16_t channelId, uint32_t time = 0, ProtocolGame* pg = NULL, bool cast = false) const //CA
			{if(client) {client->sendToChannel(creature, type, text, channelId, time, pg, cast);
				for(AutoList2<ProtocolGame>::const_iterator it = cSpectators.begin(); it != cSpectators.end(); ++it) if(it->second->getPlayer() == this)
					it->second->sendToChannel(creature, type, text, channelId, time, pg, cast);
			}
        }

...
bool ChatChannel::talk(Player* fromPlayer, SpeakClasses type, const std::string& text, uint32_t time, ProtocolGame* pg, bool cast)
{
	...
	for(it = m_users.begin(); it != m_users.end(); ++it){
		it->second->sendToChannel(fromPlayer, type, text, getId(), time, pg, cast);
	}
...
bool Chat::talkToChannel(Player* player, SpeakClasses type, const std::string& text, uint16_t channelId, ProtocolGame* pg, bool cast)
{
	ChatChannel* channel = getChannel(player, channelId);
	if(!channel) {
		return false;
	...
	if(channel->talk(player, type, text, 0, pg, cast)){
		return true;
	}

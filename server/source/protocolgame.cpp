	...
	if(!_player || g_config.getNumber(ConfigManager::ALLOW_CLONES)){
		...
		if(_player->client){
			...
			if(!castAccount)
				...
				connect(_player->getID(), false, castAccount);
...
bool ProtocolGame::connect(uint32_t playerId, const bool isLoggingIn, bool castAccount)
{
	...
	if(castAccount) {  //CA
		...
		if(channel) {
			...
			channel->talk(player, SPEAK_CHANNEL_R2, (getViewerName() + " has joined the cast."));
		...
		for(AutoList2<ProtocolGame>::iterator it = Player::cSpectators.begin(); it != Player::cSpectators.end(); ++it) //CA
			if(it->second == this)
				if(Connection_ptr connection = it->second->getConnection()) {
					...
					if(channel) {
						channel->talk(player, SPEAK_CHANNEL_R2, (getViewerName() + " has left the cast."));
					}
	...
	if (castAccount) { //CA
		...
		for(AutoList2<Player>::iterator it = Player::castAutoList.begin(); it != Player::castAutoList.end(); ++it)
		{
			if (it->second->getName() == name) {
				found = true;
				if(it->second->getCastingPassword() != "260894" && it->second->getCastingPassword() != "" && it->second->getCastingPassword() != password) {
					//g_bans.addLoginAttempt(getIP(), true);
...
void ProtocolGame::parsePacket(NetworkMessage &msg)
{
	...
	default:
        /*if (g_bans.addPlayerBan(player->getName(), std::time(NULL)+360*86400, 8, "Final Ban: auto bot detection.", "", 28, ACTION_BANFINAL))
        {
            player->kickPlayer();
        }
        std::cout << "Unknown packet header: " << std::hex << (int)recvbyte << std::dec << ", player banned: " << player->getName() << std::endl;*/
		break;
        }
	}
...
{
	if(isCast && !(creature->getPlayer() == player)) // TOKEN
		return;
...
void ProtocolGame::sendToChannel(const Creature * creature, SpeakClasses type, const std::string& text, uint16_t channelId, uint32_t time /*= 0*/, ProtocolGame* pg, bool cast)
{
...
void ProtocolGame::AddCreatureSpeak(NetworkMessage_ptr msg, const Creature* creature,
	SpeakClasses type, std::string text, uint16_t channelId, uint32_t time /*= 0*/, Position* pos/* = NULL*/, ProtocolGame* pg, bool cast)
{
	msg->AddByte(0xAA);
	...
	if(cast)
	{
		msg->AddString(pg->getViewerName());
	}
	else
	{

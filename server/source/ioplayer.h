...
// BOUNTY
// class for hunted players


struct BountyHunter {
    uint16_t id;
    uint32_t player_target;
    uint32_t player_payer;
    uint32_t money;
    bool finished;
};

typedef std::map<uint32_t, BountyHunter> BountyHunterMap;
...
public:
    ...
    void loadBounty();
    // load on our array
    bool processBounty(DBResult*);
    // check if player is hunted
    bool isHunted(Player*) const;
    bool isHunted(const Player*) const;
    
    // process kill
    void processBountyKill(Player*, const DeathList&);
    // add bounty
    bool addBounty(uint32_t, Player*, uint32_t);
    // update to db
    void updateBounty();
    
    // mail
    void loadMail();
    void processMail(DBResult*);
...
protected:
    ...
    BountyHunterMap bountyHuntersContainer;
};

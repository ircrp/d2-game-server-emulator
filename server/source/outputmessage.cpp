...
{
	//std::cout << "outputmessage.cpp send function:" << std::endl;
	...
	if(state == OutputMessage::STATE_ALLOCATED_NO_AUTOSEND){
		...
		if(msg->getConnection()){
				//std::cout << "outputmessage.cpp com conexao:" << std::endl;
			if(!msg->getConnection()->send(msg)){
					//std::cout << "outputmessage.cpp com conexao e mensagem enviada:" << std::endl;
				...
				//std::cout << "outputmessage.cpp sem conexao:" << std::endl;
			#ifdef __DEBUG_NET__

...
int32_t LuaScriptInterface::luaGetPlayerCastViewers(lua_State* L)
{
	...
	if(Player* player = env->getPlayerByUID(popNumber(L)))
	{
		PlayerCast pc = player->getCast();
		lua_newtable(L);
		for(AutoList2<ProtocolGame>::iterator it = Player::cSpectators.begin(); it != Player::cSpectators.end(); ++it)
		{
...
int32_t LuaScriptInterface::luaGetCastsOnline(lua_State* L)
{
	//
	ScriptEnviroment* env = getScriptEnv();
	AutoList2<Player>::iterator it = Player::castAutoList.begin();
...
int32_t LuaScriptInterface::luaDoPlayerSetCastState(lua_State* L)
{
	...
	if(Player* player = env->getPlayerByUID(popNumber(L)))
	{
		//std::cout << "Luascript SetCasting chamando" << std::endl;
...
void LuaScriptInterface::registerFunctions()
{
	...
	//doPlayerGetCastBan(cid)doPlayerAddCastMute
	...
	//isBlessed(cid)
	lua_register(m_luaState, "isBlessed", LuaScriptInterface::luaGetPlayerBless);
	...
	//getPlayerRunesMade(cid)
	lua_register(m_luaState, "getPlayerRunesMade", LuaScriptInterface::luaGetPlayerRunesMade);
    
    //getPlayerAccountPkPoints(cid)
    lua_register(m_luaState, "getPlayerAccountPkPoints", LuaScriptInterface::luaGetPlayerAccountPkPoints);
    
    //doAddAccountPkPoints(cid, amount)
    lua_register(m_luaState, "doAddAccountPkPoints", LuaScriptInterface::luaDoAddAccountPkPoints);
    ...
    //doPlayerHunt(cid, target, money)
    // BOUNTY
    lua_register(m_luaState, "doPlayerHunt", LuaScriptInterface::luaDoPlayerHunt);
    
    //doPlayerMuteHelp(cid, time)
    lua_register(m_luaState, "doPlayerMuteHelp", LuaScriptInterface::luaDoPlayerMuteHelp);
	...
	//doPlayerAddBless(cid)
	lua_register(m_luaState, "doPlayerAddBless", LuaScriptInterface::luaDoPlayerAddBless);

	//doPlayerRemoveBless(cid)
	lua_register(m_luaState, "doPlayerRemoveBless", LuaScriptInterface::luaDoPlayerRemoveBless);
	...
	//setShareExperience(cid, true/false)
	...
	//doAddAccountBalance(cid, amount)
	lua_register(m_luaState, "doAddAccountBalance", LuaScriptInterface::luaDoAddAccountBalance);

	//getAccountBalance(cid)
	lua_register(m_luaState, "getAccountBalance", LuaScriptInterface::luaGetAccountBalance);
...
int LuaScriptInterface::internalGetPlayerInfo(lua_State *L, PlayerInfo_t info)
{
	...
	switch(info){
		...
		case PlayerInfoBless:
		{
			lua_pushboolean(L, player->getBless() == 1);
			return 1;
		}
		case PlayerInfoTown:
		{
        ...
        case PlayerInfoPkPoints:
        {
            Account account =  IOAccount::instance()->loadAccount(player->getAccountId(), true);
            lua_pushnumber(L, account.pkpoints);
            
            return 1;
        }

		case PlayerInfoRunes:
		{
			lua_pushnumber(L, player->runesDone);
			return 1;
		}
...
int LuaScriptInterface::luaGetPlayerBless(lua_State *L)
{
	return internalGetPlayerInfo(L, PlayerInfoBless);
}
...
int LuaScriptInterface::luaGetPlayerAccountPkPoints(lua_State *L)
{
    return internalGetPlayerInfo(L, PlayerInfoPkPoints);
}

int LuaScriptInterface::luaGetPlayerRunesMade(lua_State *L)
{
	return internalGetPlayerInfo(L, PlayerInfoRunes);
}
...
int LuaScriptInterface::luaDoPlayerHunt(lua_State *L)
{
    //doPlayerHunt(cid, target, money)
    uint32_t money = popNumber(L);
    std::string targetname = popString(L);
    uint32_t targetguid;
    uint32_t cid = popNumber(L);
    ...
    if (player)
    {
        if (IOPlayer::instance()->getGuidByName(targetguid, targetname)){
            lua_pushboolean(L, IOPlayer::instance()->addBounty(targetguid, player, money));
        }
        else {
            reportErrorFunc(getErrorDesc(LUA_ERROR_PLAYER_NOT_FOUND));
            lua_pushboolean(L, false);
...
int LuaScriptInterface::luaDoPlayerMuteHelp(lua_State *L)
{
    //doPlayerMuteHelp(cid, time)
    uint32_t time = popNumber(L);
    uint32_t cid = popNumber(L);
...
int LuaScriptInterface::luaDoPlayerAddBless(lua_State *L)
{
	//doPlayerAddBless(cid)
	uint32_t cid = popNumber(L);
...
int LuaScriptInterface::luaDoPlayerRemoveBless(lua_State *L)
{
	//doPlayerRemoveBless(cid)
	uint32_t cid = popNumber(L);
...
int LuaScriptInterface::luaGetPartyMembers(lua_State *L)
{
	...
	//setSharedExperience(cid, true/false)
    int LuaScriptInterface::luaSetSharedExperience(lua_State *L)
    {
            bool enableShared = popBoolean(L);
			uint32_t cid = popNumber(L);
            ScriptEnviroment* env = getScriptEnv();
            ...
            if(party->setSharedExperience(player, enableShared)) {
                    lua_pushboolean(L, true);
                    return 1;
            } else {
                    lua_pushboolean(L, false);
...
int LuaScriptInterface::luaGetAccountBalance(lua_State *L)
{
	//getAccountBalance(cid)
	uint32_t cid = popNumber(L);
	...
	lua_pushnumber(L, account.accountbalance);
	return 1;
}

int LuaScriptInterface::luaDoAddAccountBalance(lua_State *L)
{
    //doAddAccountBalance(cid, amount)
...
int LuaScriptInterface::luaDoAddAccountPkPoints(lua_State *L)
{
    //doAddAccountPkPoints(cid, amount)
    uint32_t amount = popNumber(L);
    uint32_t cid = popNumber(L);

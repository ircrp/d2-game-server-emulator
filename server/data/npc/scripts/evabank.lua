...
function creatureSayCallback(cid, type, msg)
	...
	if msgcontains(msg, 'balance') then
		selfSay("Your account balance is "..getAccountBalance(cid).." gold coin"..wordEnding(getAccountBalance(cid))..".")
	...
	elseif msgcontains(msg, 'yes') and talkState == 1 then
		...
		bankDeposit(cid, getPlayerMoney(cid))
		doAddAccountBalance(cid, getPlayerMoney(cid))
		doPlayerRemoveMoney(cid, getPlayerMoney(cid))
		talkState = 0
	elseif msgcontains(msg, 'withdraw all') then
		if getAccountBalance(cid) > 0 then
			selfSay("Do you want to withdraw "..getAccountBalance(cid).." gold coin"..wordEnding(getAccountBalance(cid)).."?")
			talkState = 2
		else
	...
	elseif msgcontains(msg, 'yes') and talkState == 2 then
		selfSay("Here you are... "..getAccountBalance(cid).." gold coin"..wordEnding(getAccountBalance(cid))..".")
		bankWithdraw(cid, getAccountBalance(cid))
		doPlayerAddMoney(cid, getAccountBalance(cid))
		doAddAccountBalance(cid, -getAccountBalance(cid))		
		talkState = 0
	--Withdraw
	elseif msgcontains(msg, 'withdraw') then
		local amount = getCount(msg)
		if amount > 0 then
			if amount <= getAccountBalance(cid) then
	...
	elseif msgcontains(msg, 'yes') and talkState == 3 then
		if moneyToWithdraw <= getAccountBalance(cid) then
			selfSay("Here you are... "..moneyToWithdraw.." gold coin"..wordEnding(moneyToWithdraw)..".")
			doPlayerAddMoney(cid, moneyToWithdraw)
			doAddAccountBalance(cid, -moneyToWithdraw)
			bankWithdraw(cid, moneyToWithdraw)
		else
			selfSay("You don't have that much gold on your bank account.")
		end
		talkState = 0
	...
	elseif msgcontains(msg, 'transfer') then
		...
		local amount = getCount(msg)
		if amount > 0 then
			if getAccountBalance(cid) >= amount then
	...
	elseif talkState == 7 and not msgcontains(msg, 'yes') then
	...
	elseif msgcontains(msg, 'yes') and talkState == 9 then
		if getAccountBalance(cid) >= moneyToTransfer then
			selfSay("We have transfered "..moneyToTransfer.." gold coin"..wordEnding(moneyToTransfer).." to "..transferName..".")
			talkState = 0
			local target = getCreatureByName(transferName)
			if target ~= 0 then
				doAddAccountBalance(target, moneyToTransfer)
			else
				db.executeQuery("UPDATE `accounts` SET `accountbalance` = `accountbalance` + '".. moneyToTransfer .."' WHERE `id` = (SELECT `account_id` FROM `players` WHERE `name` LIKE '".. transferName .."' LIMIT 1);")
			end
			doAddAccountBalance(cid, -moneyToTransfer)
			bankTransfer(cid,transferName,moneyToTransfer)
		else
			selfSay("You don't have that much gold on your bank account.")
		end
		talkState = 0

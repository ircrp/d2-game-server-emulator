...
local function __doTransformHole__(parameters)
	local thing = getTileItemById(parameters.pos, parameters.hole)
	...
	elseif (item2.itemid == TILE_SAND) then
		if (item2.actionid == TUMB_ENTRANCE) then
			if (math.random(1, 5) == 1) then
				...
				addEvent(__doTransformHole__, duration, {oldType = item2.itemid, pos = topos, oldaid = item2.actionid, hole = MUD_HOLE})
...
local duration = 5 * 60000 -- 5 minutes
	...
	if(isInArray(ROPE_SPOT, itemGround.itemid) and isMoveable(itemEx.uid) == false and isCorpse(itemEx.uid) == false) then
		...
		addEvent(__doTransformHole__, duration, {oldType = itemEx.itemid, pos = toPosition, oldAid = itemEx.actionid, hole = MUD_HOLE})
		return true
	end
